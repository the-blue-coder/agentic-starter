...
> Assumes the `symfony-nextjs-contabo` stack (Symfony + Next.js + Docker + Contabo + nginx - see `.context/architecture.md` / `.context/infra.md`) - adapt or skip on a different stack.

Set up rolling zero-downtime deploy on a project that is **already live in production** with a single instance per service - or add more instances to a project that already has rolling deploy enabled. Reference implementation: this project's own `.context/infra.md` "Topology B" section, and the exact steps below (originally validated in production on another project using this same pattern).
